...
ENV OCELOT_HOST=ocelot
ENV OCELOT_PORT=2710

# Install git, ca-certificates, rsync, libz-dev, netcat, ssmtp and libmcrypt-dev
RUN apt-get update -y && \
    apt-get install --no-install-recommends --no-install-suggests -y git ca-certificates netcat rsync libz-dev ssmtp libmcrypt-dev && \
...
# Install memcache, mcrypt and mysqli, and configure ssmpt
...
COPY gazelle-entrypoint.sh /usr/local/bin/
